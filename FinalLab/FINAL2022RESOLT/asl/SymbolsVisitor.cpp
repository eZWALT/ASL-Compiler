...
antlrcpp::Any SymbolsVisitor::visitType(AslParser::TypeContext *ctx){
  ...
  if (ctx->ARRAY()){
    ...
    TypesMgr::TypeId t = Types.createArrayTy(atoi(ctx->INTVAL(0)->getText().c_str()),telem);
    putTypeDecor(ctx, t);
  } 
  else if(ctx->MATRIX()){
    visit(ctx->basic_type());

    uint rows = atoi(ctx->INTVAL(0)->getText().c_str());
    uint cols = atoi(ctx->INTVAL(1)->getText().c_str());
    TypesMgr::TypeId telem = getTypeDecor(ctx->basic_type());    
    TypesMgr::TypeId t = Types.createMatrixTy(rows,cols,telem);
    putTypeDecor(ctx,t);
