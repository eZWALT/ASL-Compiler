...
antlrcpp::Any TypeCheckVisitor::visitFactorial(AslParser::FactorialContext *ctx){
  ...
  if(not Types.isErrorTy(exprType) and not Types.isIntegerTy(exprType)){
...
antlrcpp::Any TypeCheckVisitor::visitMatrixIdent(AslParser::MatrixIdentContext *ctx){
  DEBUG_ENTER();
  visit(ctx->ident());
  visit(ctx->expr(0));
  visit(ctx->expr(1));

  TypesMgr::TypeId texp1 = getTypeDecor(ctx->expr(0));
  TypesMgr::TypeId texp2 = getTypeDecor(ctx->expr(1));
  TypesMgr::TypeId t = getTypeDecor(ctx->ident());
  bool isLvalue = getIsLValueDecor(ctx->ident());

  //si no es error s'inicialitza a 1
  bool isMatrix = not Types.isErrorTy(t);
  TypesMgr::TypeId decoration = t;

  if((not Types.isErrorTy(t)) and (not Types.isMatrixTy(t))){
  ...
  if ((not Types.isErrorTy(texp1)) and (not Types.isIntegerTy(texp1))){
    Errors.nonIntegerIndexInMatrixAccess(ctx->expr(0));    
  }
  else if(((not Types.isErrorTy(texp2)) and (not Types.isIntegerTy(texp2)))){
    Errors.nonIntegerIndexInMatrixAccess(ctx->expr(1));
  }

  if(isMatrix){
    decoration = Types.getMatrixElemType(t);
  }
...
antlrcpp::Any TypeCheckVisitor::visitMatrix(AslParser::MatrixContext *ctx){
  DEBUG_ENTER();
  visit(ctx->ident());
  visit(ctx->expr(0));
  visit(ctx->expr(1));
  ...
  if (not Types.isErrorTy(t) && not Types.isMatrixTy(t)){
    Errors.nonMatrixInMatrixAccess(ctx);
    putTypeDecor(ctx, Types.createErrorTy());  
  } 

  if(Types.isMatrixTy(t)) putTypeDecor(ctx, Types.getMatrixElemType(t));
