...
antlrcpp::Any CodeGenVisitor::visitPotency(AslParser::PotencyContext *ctx){
  DEBUG_ENTER();
  ...
  TypesMgr::TypeId tbase = getTypeDecor(ctx->expr(0));

  std::string expTmp = "%" + codeCounters.newTEMP();
  std::string result = "%" + codeCounters.newTEMP();
  std::string constZero="%"+ codeCounters.newTEMP();
  std::string boolean = "%" + codeCounters.newTEMP();
  std::string constOne ="%"+ codeCounters.newTEMP();


  std::string potLabel = codeCounters.newLabelWHILE();
  std::string START    = "StartExp_" + potLabel;
  std::string END      = "EndExp_" + potLabel;

  if (Types.isFloatTy(tbase)){
    code = code || instruction::LOAD(expTmp, addr2) || instruction::FLOAD(result, "1.0") || 
          instruction::ILOAD(constZero, "0") || instruction::ILOAD(constOne, "1") || instruction::LABEL(START)  || 
          instruction::LT(boolean, constZero, expTmp) || instruction::FJUMP(boolean, END) || instruction::FMUL(result, result, addr1) || 
          instruction::SUB(expTmp, expTmp, constOne) || instruction::UJUMP(START) || instruction::LABEL(END);
  }
  else {
    code = code || instruction::LOAD(expTmp, addr2) || instruction::ILOAD(result, "1") || 
      instruction::ILOAD(constZero, "0") || instruction::ILOAD(constOne, "1") || instruction::LABEL(START)  || 
      instruction::LT(boolean, constZero, expTmp) || instruction::FJUMP(boolean, END) || instruction::MUL(result, result, addr1) || 
      instruction::SUB(expTmp, expTmp, constOne) || instruction::UJUMP(START) || instruction::LABEL(END) || instruction::FLOAT(result, result);
...
antlrcpp::Any CodeGenVisitor::visitMap(AslParser::MapContext *ctx){
  ...
  //instructionList &   code2 = codAt2.code;
  //instructionList &&   code = code1 || code2;
  TypesMgr::TypeId toriginA = getTypeDecor(ctx->ident(0));
  TypesMgr::TypeId torigin  = Types.getArrayElemType(toriginA);

  TypesMgr::TypeId tfunc  = getTypeDecor(ctx->ident(2));
  TypesMgr::TypeId tinput = Types.getFuncParamsTypes(tfunc)[0];
  TypesMgr::TypeId toutput= Types.getFuncReturnType(tfunc);

  TypesMgr::TypeId tdest  = Types.getArrayElemType(getTypeDecor(ctx->ident(1)));
  uint length = Types.getArraySize(toriginA);

  std::string lengthStr = std::to_string(length);

  std::string lengthTmp = "%" + codeCounters.newTEMP();
  std::string idx = "%" + codeCounters.newTEMP();
  std::string origin = "%" + codeCounters.newTEMP();
  std::string dest = "%" + codeCounters.newTEMP();
  std::string boolean = "%" + codeCounters.newTEMP();
  std::string constOne = "%" + codeCounters.newTEMP();

  std::string labMap = codeCounters.newLabelWHILE();
  std::string START = "StartMap_" + labMap;
  std::string END   = "EndMap_" + labMap;

  instructionList && code = instruction::ILOAD(constOne, "1");

  code = code || instruction::ILOAD(idx, "0") || instruction::ILOAD(lengthTmp, lengthStr) || instruction::LABEL(START) ||
              instruction::LT(boolean, idx, lengthTmp) || instruction::FJUMP(boolean, END) || instruction::LOADX(origin, addr1, idx) ||instruction::PUSH() ;

  if (not Types.equalTypes(torigin, tinput)) code = code || instruction::FLOAT(origin, origin);

  code = code || instruction::PUSH(origin) || instruction::CALL(ctx->ident(2)->getText()) || instruction::POP() || instruction::POP(dest);

  if (not Types.equalTypes(tdest, toutput)) code = code || instruction::FLOAT(dest, dest); 

  code = code || instruction::XLOAD(addr2, idx, dest) || instruction::ADD(idx, idx, constOne) || instruction::UJUMP(START) || instruction::LABEL(END);
